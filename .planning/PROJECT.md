...
| SHA-pin both workflows + `pinact` verify-pins CI gate — v3.1.0 (SEC-01) | A floating `@v4` tag is mutable and a bare SHA can point at a fork/wrong commit; the `id-token: write` publish job is the highest-privilege surface. Pin to 40-char SHA + `# vX.Y.Z` comment and verify the SHA↔tag resolution in CI (assert the value, not the shape). | ✓ Validated in v3.1.0 (pinact gate green on first live run, Stage 1/2 pushes) |
| Paired OIDC floors asserted at runtime — v3.1.0 (SEC-01) | npm OIDC trusted publishing needs npm ≥11.5.1 on Node ≥22.14.0; a shape-only declaration can still drift. Exact-pin both and assert numerically (`sort -V`, lexical-trap-proof) so a below-floor runner fails closed. | ✓ Validated in v3.1.0 |
| Hand-rolled `isVersionRequest()` predicate, no parser dep — v3.1.0 (CLI-06) | One flag does not justify a `commander`/`yargs` dependency in the mcp bin. Pre-transport predicate (`--version`/`--version=…`/`-v`/`-V`, case-sensitive) preserves the exact-string `verify-tarball` contract. | ✓ Validated in v3.1.0 (8 behavioral smoke cases, handshake fall-through proof) |
| Parameterize `seed()` with `toolkitVersion` (Option A) — v3.1.0 (BUILD-01) | The bundled `seed.ts` cannot read `package.json` at runtime (`noExternal` inlining resolves `import.meta.url` to the consumer's dist). Caller passes its runtime version in; `verify-tarball` asserts the seed-path version *value* through each real consumer surface. | ✓ Validated in v3.1.0 (last hardcoded version literal eliminated) |
| Path-shape-only `looksLikeProject` tightening, wired to detect — v3.1.0 (CORE-15) | Filter system/home/drive/AppData/other-user roots by path shape, NOT by a `.git`/`package.json` marker (D-01: marker-less plain folders must stay discoverable). Shared core so audit and detect behave identically. | ✓ Validated in v3.1.0 (12-test regression-lock: root-rejection + plain-folder discovery) |
| Additive `encodedName + '--'` decode branch + verify-then-return — v3.1.0 (CORE-16) | Fix the `buildCandidates` trailing-hyphen-strip asymmetry additively (every passing shape untouched), NOT by widening the calibrated `encode()` character class. `decode()` returns only a candidate that re-encodes to the input hash; `encode()` byte-unchanged. | ✓ Validated in v3.1.0 (9×5 char×position matrix; OneDrive fix non-regressed; WR-01 collision edge → backlog 999.9) |
...
---
*Last updated: 2026-06-30 after v3.1.0 "Hardening and Hygiene" milestone — full evolution review at close. All five requirements validated (SEC-01/CLI-06 Phase 21; BUILD-01/CORE-15 Phase 22; CORE-16 Phase 23) and moved to Validated; six v3.1.0 key decisions logged. Packages bumped to 3.1.0 with CI green (3-OS + pinact); the v3.1.0 release tag is pending push — pushing it triggers the OIDC + SLSA-provenance publish of @localground/mcp@3.1.0 + @localground/cli@3.1.0. Backlog carries 999.5 (CLI-05 → v3.2.0), 999.8, 999.9. Next: /gsd-new-milestone.*
